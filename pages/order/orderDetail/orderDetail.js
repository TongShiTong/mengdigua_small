...
Page({
  ...
  copyOrder(){
    wx.setClipboardData({
      data: this.data.detailData.order_no,
      success:()=>{
        wx.showToast({
          title: '复制成功！',
        })
  ...
  getLog() {
    let self = this;
    http.HttpRequst(false, '/order/order-api/logistic', 2, '', {
        token: app.globalData.token,
  ...
  jumpDetail(e) {
    console.log(147,e)
    let id = e.currentTarget.dataset.id;
    let shop_id = e.currentTarget.dataset.shop_id;//接口未反shop_id,
    wx.navigateTo({
